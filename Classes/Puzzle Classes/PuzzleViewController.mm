...
- (void) setupBackgrounds {
	...
	NSString *imagePathHorisontal = [bundle pathForResource:[NSString stringWithFormat:@"puzzlebkg_horisontal"] ofType:@"png"];
	UIImageView *horisontalLeftBKGView = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:imagePathHorisontal]];
	[self.view addSubview:horisontalLeftBKGView];
	horisontalLeftBKGView.center = CGPointMake((self.view.frame.size.width)/2+4, (self.view.frame.size.height)/2-14);
	[self.view sendSubviewToBack:horisontalLeftBKGView];
	[horisontalLeftBKGView release];
	
}
-(void) changePuzzleDifficulty:(int) difficulty {
	//FLURRY
	[[cdaAnalytics sharedInstance] trackEvent:[NSString stringWithFormat:@"PUZZLE started with level of difficulty: %i", [myParent getLevelOfDifficulty]]];
	//
	//FLURRY
	[[cdaAnalytics sharedInstance] trackEvent:[NSString stringWithFormat:@"PUZZLE started with puzzle no: %i", currentSelectedJigsaw]];
	//[[cdaAnalytics sharedInstance] trackEvent:@"Solving_puzzle" timed:YES];
	//
